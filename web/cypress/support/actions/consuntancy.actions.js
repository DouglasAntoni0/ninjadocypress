Cypress.Commands.add('fillConsultancyForm',(form) => {
    cy.get('#name').type(form.name)
        ...
        cy.get('input[placeholder="(00) 00000-0000"]').type(form.phone).should('have.value', '(11) 99999-1000')

        cy.contains('label', 'Tipo de Consultoria').parent().find('select').select(form.consultancyType)

        if (form.personType === 'cpf') {
            cy.contains('label', 'Pessoa Física').find('input').click().should('be.checked')
            cy.contains('label', 'CPF').parent().find('input').type(form.document).should('have.value', '123.456.789-10',)
        } if (form.personType === 'cnpj') {
            cy.contains('label', 'Pessoa Jurídica').find('input').click().should('be.checked')
            cy.get('#document').type(form.document).should('have.value', '12.728.543/0001-09',)
        }
...
Cypress.Commands.add('validateConsultancySuccessModal', () => {
